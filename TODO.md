...
- [x] `(untracked)` ✅ **Blog no ar** (2026-09-01) em **https://blog.pacafinance.com.br** — projeto Vercel `paca-blog` (conta mariaeduardaaf), root `apps/blog`, production branch `feat/hardening-and-blog` (push = deploy), DNS na Cloudflare (CNAME `blog`, DNS only), HTTPS válido. Já tem: capa única por artigo, diagramas nos textos, card de compartilhamento por artigo, captura de e-mail LGPD, `/privacidade`, `/descadastro`, `ads.txt` (aguardando publisher ID).
- [ ] `(untracked)` 🆕 **Domínio próprio para o app web** — hoje é `paca-web-twmh.vercel.app`, que não passa confiança num app de dinheiro. Alvo: **`app.pacafinance.com.br`**. Ordem: **(1)** adicionar o domínio no projeto Vercel do app (está na conta **madualvesfr**, não na nova) — antes, conferir qual dos dois projetos duplicados (`paca-web` × `paca-web-twmh`) está em uso; **(2)** CNAME `app` → `cname.vercel-dns.com` na Cloudflare (nuvem cinza); **(3)** Supabase → Authentication → URL Configuration: incluir o novo endereço no *Site URL* e nos *Redirect URLs* (senão o link de recuperação de senha e o login social apontam para o endereço velho); **(4)** IA troca as **16 referências** no código (8 artigos do blog, `apps/blog/src/consts.ts`, `PrivacyPage`/`TermsPage` do app, `blog-unsubscribe`, `SUBMISSION.md` — que alimenta as URLs enviadas às lojas).
- [ ] `(untracked)` 🆕 **Monetização e indexação do blog:** **(a)** definir a conta AdSense — as contas existentes são de terceiros ou do perfil espanhol em euro (ver histórico 2026-09-01); com o `pub-…` certo, IA preenche `ads.txt` + env `PUBLIC_ADSENSE_CLIENT` e o anúncio liga sozinho; **(b)** Search Console + submeter sitemap; **(c)** tag Caju + Meta Pixel (pontos já marcados no `BaseLayout`); **(d)** redirect do apex `pacafinance.com.br`. Estratégia: `docs/arbitragem/`.
