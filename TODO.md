...
- [ ] `(untracked)` 🆕 **Blog — go-live** (2026-09-01): ✅ NO AR em https://paca-blog.vercel.app — projeto `paca-blog` (conta Vercel mariaeduardaaf), root `apps/blog`, production branch `feat/hardening-and-blog`, GitHub conectado (push = deploy), domínio `blog.pacafinance.com.br` adicionado (aguarda DNS), deployment protection desligada. **Falta:** **(a)** registro do `pacafinance.com.br` concluir no Registro.br → criar CNAME `blog` → `cname.vercel-dns.com`; **(b)** conta AdSense (CNPJ) e, aprovada, publisher ID em `public/ads.txt` + env `PUBLIC_ADSENSE_CLIENT` na Vercel (anúncio liga sozinho); **(c)** Search Console + sitemap; **(d)** tag Caju + Meta Pixel (pontos marcados no `BaseLayout`). Estratégia: `docs/arbitragem/`.
