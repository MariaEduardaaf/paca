...
- [ ] `(untracked)` 🆕 **Deploy do blog** (decisões 2026-09-01: domínio `blog.pacafinance.com.br` ✅ trocado no código; monetização = AdSense agora, AdSeleto depois; deploy direto da branch): **(a)** registrar `pacafinance.com.br` no Registro.br; **(b)** criar projeto Vercel com root `apps/blog` e production branch = `feat/hardening-and-blog`; **(c)** apontar DNS do subdomínio pra Vercel; **(d)** criar conta AdSense (CNPJ) e, aprovada, colar o publisher ID em `apps/blog/public/ads.txt`; **(e)** Google Search Console + submeter sitemap. Estratégia completa: `docs/arbitragem/`.
